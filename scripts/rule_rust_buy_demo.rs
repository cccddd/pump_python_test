...
impl Default for BuyConditionsConfig {
    fn default() -> Self {
        Self {
            ...
            // 条件16: 近N秒内交易单数量
            recent_trade_count_check_mode: CheckMode::Debug,
...
/// 检查买入条件（只有 Online 模式的条件才会真正过滤，Debug 模式不影响买入）
...
pub fn check_buy_conditions(
    ...
    // 条件5: 检查过滤后的前N笔交易总和 (始终检查，非debug条件)
    ...
    // 条件8a: 价格波动率 (仅 Online 模式检查)
    if cfg.price_volatility_check_mode == CheckMode::Online {
        if let Some(pv) = features.price_volatility_cv {
    ...
    // 条件8b: 时间波动率 (仅 Online 模式检查)
    if cfg.time_volatility_check_mode == CheckMode::Online {
        if let Some(tv) = features.time_volatility_cv {
    ...
    // 条件8c: 金额波动率 (仅 Online 模式检查)
    if cfg.amount_volatility_check_mode == CheckMode::Online {
        if let Some(av) = features.amount_volatility_cv {
    ...
    // 条件10: 买单数量检查 (仅 Online 模式检查)
    if cfg.buy_count_check_mode == CheckMode::Online {
        // 使用配置的 lookback count 重新计算
        let buy_count = recent_trades.iter()
    ...
    // 条件11: 卖单数量检查 (仅 Online 模式检查)
    if cfg.sell_count_check_mode == CheckMode::Online {
        let sell_count = recent_trades.iter()
    ...
    // 条件12: 大单占比检查 (仅 Online 模式检查)
    if cfg.large_trade_ratio_check_mode == CheckMode::Online {
        let lookback = cfg.large_trade_ratio_lookback.min(recent_trades.len());
        if lookback > 0 {
            let large_count = recent_trades.iter()
                .take(lookback)
    ...
    // 条件13: 小单占比检查 (仅 Online 模式检查)
    if cfg.small_trade_ratio_check_mode == CheckMode::Online {
        let lookback = cfg.small_trade_ratio_lookback.min(recent_trades.len());
        if lookback > 0 {
            let small_count = recent_trades.iter()
                .take(lookback)
    ...
    // 条件14: 连续大额买单检查 (仅 Online 模式检查)
    if cfg.consecutive_buy_check_mode == CheckMode::Online {
        let consecutive_buy = recent_trades.iter()
            .take_while(|rt| rt.trade_type == "buy" && rt.buy_amount.abs() >= cfg.consecutive_buy_threshold)
            .count();
        if consecutive_buy < cfg.consecutive_buy_min {
            reject_reason = format!("连续买单数量不足({}<{})", consecutive_buy, cfg.consecutive_buy_min);
            return (false, reject_reason);
        }
    }
    
    // 条件15: 连续大额卖单检查 (仅 Online 模式检查)
    if cfg.consecutive_sell_check_mode == CheckMode::Online {
        let consecutive_sell = recent_trades.iter()
            .take_while(|rt| rt.trade_type != "buy" && rt.buy_amount.abs() >= cfg.consecutive_sell_threshold)
            .count();
        if consecutive_sell > cfg.consecutive_sell_max {
            reject_reason = format!("连续卖单数量过多({}>{})", consecutive_sell, cfg.consecutive_sell_max);
            return (false, reject_reason);
        }
    }
    
    // 条件16: 近N秒内交易单数量 (仅 Online 模式检查)
...
impl BuyFeatures {
    pub fn to_log_string(&self) -> String {
        format!(
            "mint={} | user={} | type={} | time_create={:.2}min | time_diff={:.0}ms | price={:.10} | nowsol={:.2} | trade_sol={:.4} | filtered_cnt={} | filtered_sum={:.4} | price_cv={} | time_cv={} | amount_cv={} | price_ratio={} | buy_cnt={} | sell_cnt={} | consec_buy={} | consec_sell={} | large_ratio={} | small_ratio={} | matched_group={} | avg_profit={} | buy_sol={:.4}",
            self.mint,
            self.user,
            self.trade_type,
                    ...
                    .get(&data.trade_mint().to_string())
                {
                    ...
                    for rt in recent_trades.iter() {
                        if let Some(pv) = prev_time {
                            let diff = (pv as f64) - (rt.swap_time as f64);
                            if diff.abs() > 0.0 {
                                intervals.push(diff);
                            }
                        }
                        prev_time = Some(rt.swap_time);
                    }
                    features.time_volatility_cv = compute_cv(&intervals);

                    // 计算金额波动率
                    let amts: Vec<f64> = recent_trades.iter().map(|rt| rt.buy_amount as f64).collect();
                    features.amount_volatility_cv = compute_cv(&amts);
                    ...
                    // 计算买卖计数
                    features.buy_count = recent_trades.iter().filter(|rt| rt.trade_type == "buy").count();
                    features.sell_count = recent_trades.iter().filter(|rt| rt.trade_type == "sell").count();
                    features.consecutive_buy = recent_trades.iter().take_while(|rt| rt.trade_type == "buy").count();
                    features.consecutive_sell = recent_trades.iter().take_while(|rt| rt.trade_type == "sell").count();

                    // 计算大小单比例
                    let total = recent_trades.len();
                    if total > 0 {
                        let large = recent_trades.iter().filter(|rt| rt.buy_amount.abs() >= 1.0).count();
                        let small = recent_trades.iter().filter(|rt| rt.buy_amount.abs() < 0.1).count();
                        features.large_trade_ratio = Some(large as f64 / total as f64);
                        features.small_trade_ratio = Some(small as f64 / total as f64);
                    }
