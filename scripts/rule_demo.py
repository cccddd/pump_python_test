...
BUY_CONDITIONS_CONFIG = {
    ...
    'FILTERED_TRADES_SUM_RANGE': (-6.0, -1.0),  # 交易金额总和范围
    ...
    # 'online' - 启用此条件，进行实际过滤
    # 'debug' - 调试模式，只收集统计数据，不过滤
    'TIME_VOLATILITY_CHECK_MODE': 'online',  # 可选值: 'off', 'online', 'debug'
    ...
    # 'online' - 启用此条件，进行实际过滤
    # 'debug' - 调试模式，只收集统计数据，不过滤
    'AMOUNT_VOLATILITY_CHECK_MODE': 'online',  # 可选值: 'off', 'online', 'debug'
    ...
    'CONSECUTIVE_SELL_THRESHOLD': 0.3,  # 大额卖单阈值(SOL)，交易金额绝对值 >= 此值
...
BUY_CONDITIONS_CONFIG = {
    ...
    'TIME_DIFF_CHECK_MODE': 'online',
    'TIME_DIFF_FROM_LAST_TRADE_RANGE': (10000, 50000),
    ...
    'FILTERED_TRADES_SUM_RANGE': (-3.0, -1.0),
}
    ...
    for i in range(buy_index, len(trade_data)):
        ...
        if max_price > original_buy_price:
            ...
            if max_profit_rate < high_profit_threshold:
                # 条件3: 最大盈利 < 50% 时，回撤达到 3% 卖出
                if retracement >= retracement_low:
                    return i, f"回撤止损 (最大盈利{max_profit_rate*100:.2f}%<50%, 回撤{retracement*100:.2f}%>={retracement_low*100:.0f}%)"
            else:
                # 条件4: 最大盈利 >= 50% 时，回撤达到 5% 卖出
                if retracement >= retracement_high:
                    return i, f"回撤止损 (最大盈利{max_profit_rate*100:.2f}%>=50%, 回撤{retracement*100:.2f}%>={retracement_high*100:.0f}%)"
