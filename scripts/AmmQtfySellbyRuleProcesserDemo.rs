...
pub struct SellConditionsConfig {
    ...
    pub rebound_min_loss_pct: f64,      // 买入后最低亏损阈值百分比（跌破此值后才启用反弹检测）
    pub rebound_min_profit_pct: f64,    // 从最低价反弹的百分比阈值（(当前价-最低价)/最低价 >= 此值时卖出）
    // 活跃期高涨幅卖出
    ...
    // 近N单卖单占比卖出
    pub sell_ratio_sell_enabled: bool,        // 是否启用
    pub sell_ratio_lookback: usize,           // 向前查看的交易数量
    pub sell_ratio_threshold: f64,            // 卖单占比阈值 (0.7 = 70%)
    pub sell_ratio_min_hold_ms: u64,          // 最小持有时间（毫秒）
}
...
impl SellConditionsConfig {
    ...
    fn default() -> Self {
        Self {
            max_nowsol_sell: 320.0,
            profit_rate_sell_enabled: true,
            profit_rate_sell_threshold: 0.5,     // 盈利率 >= 50% 时直接卖出
            loss_percentage: 0.6,                // 亏损达到 60% 时触发止损
            lookback_trades_for_min_price: 3,
            retracement_low_profit: 0.5,         // 最大盈利 < 0% 时，回撤 50% 卖出
            retracement_high_profit: 0.5,        // 最大盈利 >= 0% 时，回撤 50% 卖出
            high_profit_threshold: 0.0,          // 高盈利阈值 0%
            retracement_min_count: 0,            // 拐点次数阈值
            retracement_inflection_window: 5,    // 拐点检测窗口大小
            retracement_min_hold_ms: 10000,      // 回撤止损最小持有时间 10秒
            retracement_min_profit: 0.08,        // 回撤止损最小盈利阈值 8%
            ...
            quiet_period_min_amount: 0.4,
            spike_sell_enabled: true,
            spike_lookback_ms: 800,
            spike_threshold_pct: 15.0,
            rebound_sell_enabled: true,
            rebound_min_loss_pct: 0.0,           // 买入后最低亏损阈值百分比
            rebound_min_profit_pct: 25.0,        // 从最低价反弹百分比阈值
            // 活跃期高涨幅卖出
            active_spike_sell_enabled: true,
            active_spike_window_seconds: 3,
            active_spike_min_trade_count: 20,
            active_spike_lookback_count: 20,
            active_spike_min_rise_pct: 35.0,
            // 低活跃涨幅卖出
            inactive_spike_sell_enabled: false,
            inactive_spike_window_seconds: 10,
            inactive_spike_max_trade_count: 5,
            inactive_spike_lookback_count: 20,
            inactive_spike_min_rise_pct: 12.0,
            // 近N单卖单占比卖出
            sell_ratio_sell_enabled: false,
            sell_ratio_lookback: 30,
            sell_ratio_threshold: 0.7,
            sell_ratio_min_hold_ms: 60000,
        }
    }
}
...
pub struct AmmQtfySellbyRuleProcesserDemo;

#[async_trait]
impl PipelineHandler for AmmQtfySellbyRuleProcesserDemo {
    async fn handle(&self, data: TradeContextNew) -> Option<TradeContextNew> {
        ...
        if let Some(mut indicator) = WATCH_CACHE.get_mut(data.trade_mint()) {
                ...
                .get_mut(key.as_str())
            {
                ...
                //            and min_price_during_hold > 0:
                //     rebound_pct = (current_price - min_price) / min_price
                //     if rebound_pct >= rebound_min_rebound_pct: 卖出
                // has_experienced_loss = 曾经亏损超过 rebound_min_loss_pct
                // 使用 indicator.min_price 作为持有期间最低价
                // =============================================================
                if !need_sell && cfg.rebound_sell_enabled && has_experienced_loss 
                    && indicator.min_price > 0.0 && current_price > indicator.min_price {
                    let rebound_min_rebound = cfg.rebound_min_profit_pct / 100.0;
                    let rebound_pct = (current_price as f64 - indicator.min_price as f64) 
                        / indicator.min_price as f64;
                    if rebound_pct >= rebound_min_rebound {
                        sell_reason += &format!(
                            "反弹卖出(最低亏损{:.2}%>={:.0}%,最低价{:.8},反弹{:.2}%>={:.0}%)|",
                ...
                // =============================================================
                // 条件11: 卖单占比卖出
                // Python: if sell_ratio_sell_enabled:
                //     hold_time_ms >= sell_ratio_min_hold_ms
                //     近 sell_ratio_lookback 单(买入后)中卖单占比 >= sell_ratio_threshold
                // =============================================================
                if !need_sell && cfg.sell_ratio_sell_enabled {
                    let sr_hold_time_ms = current_miltime.saturating_sub(buy_time);
                    if sr_hold_time_ms >= cfg.sell_ratio_min_hold_ms as u128 {
                        let prepare_trades = &feed.prepareTrade;
                        let mut sr_sell_count = 0usize;
                        let mut sr_total_count = 0usize;
                        for pt in prepare_trades.iter() {
                            if pt.swap_time >= current_miltime { continue; }
                            if pt.swap_time < buy_time { break; }
                            sr_total_count += 1;
                            ...
                            if sr_total_count >= cfg.sell_ratio_lookback { break; }
                        }

                        if sr_total_count >= cfg.sell_ratio_lookback {
                            let sr_ratio = sr_sell_count as f64 / sr_total_count as f64;
                            if sr_ratio >= cfg.sell_ratio_threshold {
                                sell_reason += &format!(
                                    "卖单占比卖出(近{}单卖单占比{:.1}%>={:.0}%)|",
                                    cfg.sell_ratio_lookback,
                                    sr_ratio * 100.0,
                                    cfg.sell_ratio_threshold * 100.0
                                );
                                need_sell = true;
                            }
                        }
